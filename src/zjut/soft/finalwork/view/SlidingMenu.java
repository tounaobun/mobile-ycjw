 ...
 */
package zjut.soft.finalwork.view;
...
public class SlidingMenu extends RelativeLayout {
			...
			if (mScroller.computeScrollOffset()) {
				...
				if (oldX != x || oldY != y) {
					if (mSlidingView != null) {
						...
						if (x < 0)
							bgShade.scrollTo(x + 20, y);// 閼冲本娅欓梼鏉戝閸欏啿浜�
						else
							bgShade.scrollTo(x - 20, y);// 閼冲本娅欓梼鏉戝瀹革箑浜�
						}
				}
				invalidate();
			}
	...
	public boolean hasClickLeft() {
		return hasClickLeft;
	}
	
	public boolean hasClickRight() {
		return hasClickRight;
	}
